...
4×2 尺寸检查使用 Android 10、11、14 原生文字排版，覆盖默认/缩放占格、260×116dp 最小布局、
常规布局、横竖屏、多个 Widget 独立缩放、各支持语言与放大字号，防止速率和本次流量被裁剪。
...
已授权按阶段提交的实现任务，在阶段验证通过后提交本阶段实际修改的仓库；跨仓库阶段
分别提交相关改动，不要求为未变更的仓库创建提交，也不纳入其它任务的未提交改动。
普通分析、审查或文档请求不隐含提交、推送或创建 PR。VCore 只有在其自身合同确实变化时才修改。
