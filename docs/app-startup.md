...
4. 首次初始化只完成隐私确认和地区/网卡配置，具体交互见 [首次初始化](#首次初始化)；不申请 VPN 权限、不添加服务器或启动 VPN。
5. 主 Shell 以 `ServiceManager` 作为唯一就绪门，完成存储、Geodata、平台前置条件及协调器初始化后才构建业务页面；失败保留重试入口。协调器按 [权限流程](#平台前置条件与权限)读取 VPN 状态并完成必要授权，不依赖 Setup 或服务器数据，也不启动 VPN。托盘在服务初始化阶段创建、全部就绪后刷新；连接页只管理自身数据和可见期间的流量读取。
...
已准备数据只做完整性检查，不等待或中断后台测速；普通启动不暂停后台模块，清空流程才暂停。

## 首次初始化

`SetupService` 复用存储与 Geodata 准备能力，只有隐私确认和合并配置两步，不自动前进。
配置页点击“进入首页”才保存；不读取服务器列表，也不依赖测速。

- Windows/Linux 必须明确选择当前存在的出口网卡，不默认选第一张；选中即返回配置页，其它平台不显示此项。
- 地区复用智能路由的已安装 Geodata 列表、搜索、单选、清空与“完成”。识别只预选候选值，不把默认 CN 当作识别结果。
- “完成”只更新草稿，返回或取消不改选择。不选择地区保留原配置；主动清空并确认则保存空列表。
- 地区与网卡一起保存，成功后才写完成标记并导航；失败保留草稿。隐私、网卡和地区校验不因已有完成标记而跳过。
- 启动入口只根据隐私确认和首次运行标记恢复欢迎、配置或主界面，不持久化步骤序号；标记不限制已进入 Setup 的配置保存。
