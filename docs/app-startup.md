...
4. 首次初始化通过 `SetupService` 复用存储、Geodata、平台前置条件和权限能力。首次初始化必须完成 VPN 授权及适用的 Android 局域网授权；iOS Debug 不查询或设置系统 VPN，Setup 将该权限视为不需要。Windows/Linux 还必须明确选择出口网卡。国家/区域和添加服务器可由用户主动跳过，已有服务器时仍显示添加步骤。Setup 不启动 VPN。
...
丢失时仍通过文件访问队列重建，部分损坏保持报错。Geodata 不关闭全局维护门；只有恢复
备份和清空数据会阻止新任务并等待在途任务结束，具体访问协调见 [数据管理](data-management.md)。
...
除 iOS Debug 外，首次初始化仍必须取得 VPN 授权才能完成；这项产品要求不改变正常启动的只读检查语义。
iOS Debug 的 Setup 例外不更改正式构建及其他平台的权限要求。iOS 模拟器还在原生
权限查询边界直接返回“不需要”，不调用不可用的 NetworkExtension VPN preferences IPC，
避免进入首页或回到前台时因权限查询失败阻断初始化；真机仍执行正常的只读查询。
