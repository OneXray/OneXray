...
4. 首次初始化通过 `SetupService` 复用存储、Geodata、平台前置条件和权限能力。首次初始化必须完成 VPN 授权及适用的 Android 局域网授权；Windows/Linux 还必须明确选择出口网卡。国家/区域和添加服务器可跳过，已有服务器时跳过添加。Setup 不启动 VPN。
...
每个步骤满足条件后自动前进，不再要求额外点击“继续”或“前往首页”：隐私仍需主动同意；
系统准备、VPN 授权和必要的网卡选择完成后进入地区步骤；地区识别成功或手动选定后保存
直连地区并前进，不能把默认 CN 当成识别结果。地区和网卡列表选中即返回，无需再次确认。
地区无法识别时可手动选择或跳过；节点导入后只要已有可用节点即完成初始化，否则保留
添加入口和“稍后添加”。失败停留在对应步骤，不自动跳过必要条件，也不启动 VPN。
