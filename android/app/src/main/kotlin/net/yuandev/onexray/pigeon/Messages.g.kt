...
data class BackupLocation (
 ...
 {
  companion object {
    fun fromList(pigeonVar_list: List<Any?>): BackupLocation {
      val identifier = pigeonVar_list[0] as String
      val label = pigeonVar_list[1] as String
      return BackupLocation(identifier, label)
    }
  }
  fun toList(): List<Any?> {
    return listOf(
  ...
  override fun equals(other: Any?): Boolean {
    ...
    val other = other as BackupLocation
    return MessagesPigeonUtils.deepEquals(this.identifier, other.identifier) && MessagesPigeonUtils.deepEquals(this.label, other.label)
  }
  ...
  override fun toString(): String {
    return "BackupLocation(identifier=$identifier, label=$label)"
  }
}
...
/** Generated interface from Pigeon that represents a handler of messages from Flutter. */
interface BackupHostApi {
  fun selectBackupFile(create: Boolean, callback: (Result<BackupLocation?>) -> Unit)
  fun requireBackupAccess(identifier: String, callback: (Result<Unit>) -> Unit)
  fun releaseBackupFile(identifier: String, callback: (Result<Unit>) -> Unit)

  companion object {
    /** The codec used by BackupHostApi. */
    val codec: MessageCodec<Any?> by lazy {
      MessagesPigeonCodec()
    }
    /** Sets up an instance of `BackupHostApi` to handle messages through the `binaryMessenger`. */
    @JvmOverloads
    fun setUp(binaryMessenger: BinaryMessenger, api: BackupHostApi?, messageChannelSuffix: String = "") {
      val separatedMessageChannelSuffix = if (messageChannelSuffix.isNotEmpty()) ".$messageChannelSuffix" else ""
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.onexray.BackupHostApi.selectBackupFile$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val createArg = args[0] as Boolean
            api.selectBackupFile(createArg) { result: Result<BackupLocation?> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(MessagesPigeonUtils.wrapError(error))
              } else {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.onexray.BackupHostApi.requireBackupAccess$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val identifierArg = args[0] as String
            api.requireBackupAccess(identifierArg) { result: Result<Unit> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(MessagesPigeonUtils.wrapError(error))
              } else {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.onexray.BackupHostApi.releaseBackupFile$separatedMessageChannelSuffix", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val identifierArg = args[0] as String
            api.releaseBackupFile(identifierArg) { result: Result<Unit> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(MessagesPigeonUtils.wrapError(error))
              } else {
