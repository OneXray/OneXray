...
struct BackupLocation: Hashable, CustomStringConvertible {
  ...
  static func fromList(_ pigeonVar_list: [Any?]) -> BackupLocation? {
  ...
  static func == (lhs: BackupLocation, rhs: BackupLocation) -> Bool {
    if Swift.type(of: lhs) != Swift.type(of: rhs) {
      return false
    }
    return MessagesPigeonInternal.deepEquals(lhs.identifier, rhs.identifier) && MessagesPigeonInternal.deepEquals(lhs.label, rhs.label)
  }

  func hash(into hasher: inout Hasher) {
    hasher.combine("BackupLocation")
  ...
  public var description: String {
    return "BackupLocation(identifier: \(String(describing: identifier)), label: \(String(describing: label)))"
  }
}
...
/// Generated protocol from Pigeon that represents a handler of messages from Flutter.
protocol BackupHostApi {
  func selectBackupFile(create: Bool, completion: @escaping (Result<BackupLocation?, Error>) -> Void)
  func requireBackupAccess(identifier: String, completion: @escaping (Result<Void, Error>) -> Void)
  func releaseBackupFile(identifier: String, completion: @escaping (Result<Void, Error>) -> Void)
}

/// Generated setup class from Pigeon to handle messages through the `binaryMessenger`.
class BackupHostApiSetup {
  static var codec: FlutterStandardMessageCodec { MessagesPigeonCodec.shared }
  /// Sets up an instance of `BackupHostApi` to handle messages through the `binaryMessenger`.
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: BackupHostApi?, messageChannelSuffix: String = "") {
    let channelSuffix = messageChannelSuffix.count > 0 ? ".\(messageChannelSuffix)" : ""
    let selectBackupFileChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.onexray.BackupHostApi.selectBackupFile\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      selectBackupFileChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let createArg = args[0] as! Bool
        api.selectBackupFile(create: createArg) { result in
          switch result {
          case .success(let res):
            reply(wrapResult(res))
          case .failure(let error):
      ...
      selectBackupFileChannel.setMessageHandler(nil)
    }
    let requireBackupAccessChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.onexray.BackupHostApi.requireBackupAccess\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      requireBackupAccessChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let identifierArg = args[0] as! String
        api.requireBackupAccess(identifier: identifierArg) { result in
          switch result {
          case .success:
      ...
      requireBackupAccessChannel.setMessageHandler(nil)
    }
    let releaseBackupFileChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.onexray.BackupHostApi.releaseBackupFile\(channelSuffix)", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      releaseBackupFileChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let identifierArg = args[0] as! String
        api.releaseBackupFile(identifier: identifierArg) { result in
          switch result {
          case .success:
            reply(wrapResult(nil))
          case .failure(let error):
      ...
      releaseBackupFileChannel.setMessageHandler(nil)
    }
  }
}
