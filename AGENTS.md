...
The app manages nodes, subscriptions, Xray Profiles, Multi-node Outbounds, Raw Json configs, and GeoData. Before startup, it composes and writes the final runtime Xray JSON. Production builds use TUN on Android, Apple, and Linux, and a local SOCKS inbound on Windows. Proxy mode is an in-memory iOS Debug-only tool.
...
After the final runtime Xray JSON is written, Android, Apple, and Linux enter the platform VPN / TUN startup path. Windows starts the packaged Core as a normal user process and exposes its loopback SOCKS inbound. On iOS Debug builds only, Proxy mode starts local Xray core without the platform VPN; only the internal `pingIn` inbound is App-managed.
...
Windows and Linux desktop builds use Dart FFI, platform runners, and the packaged `OneXrayCore` binary to start core. Windows starts Core without elevation; Linux grants the capabilities required by TUN.
