...
The native host reports VPN state. `run/start.json` contains the current native request plus only the App metadata needed to explain and protect a host-confirmed connection; it is not a history or a source of VPN truth. Foreground traffic reads native Xray metrics; libXray periodically overwrites only the current session and exposes it through authenticated loopback HTTP. App owns cumulative totals and one settled-session watermark; it never reads libXray session files or uses an Apple statistics message bridge. Missing an entire unobserved session is acceptable. Read `docs/xray-configuration.md` before changing statistics transport or accounting. Before every actual desktop VPN start, clear `run/core-inputs`, create one unique immutable input directory for the new Core process, and keep the Windows VCore snapshot token. Packaged Windows uses VCore bridge revision 3 and its Session Host Job Object; statistics require no VCore changes. Preserve iOS Debug-only local proxy isolation from normal UI and business state.
