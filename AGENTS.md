...
The native host reports VPN state. Foreground traffic reads native Xray metrics; libXray periodically saves only the current session and exposes saved snapshots through authenticated loopback HTTP. App owns cumulative totals and reset watermarks; it never reads libXray session files or uses an Apple statistics message bridge. Offline cached statistics do not identify the running plan. Read `docs/xray-configuration.md` before changing statistics transport or accounting. Packaged Windows uses VCore bridge revision 3 and its Session Host Job Object; statistics require no VCore changes. Preserve iOS Debug-only local proxy isolation from normal UI and business state.
