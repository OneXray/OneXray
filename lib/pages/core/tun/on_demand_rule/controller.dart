...
class OnDemandRulePageState {
  ...
  OnDemandRulePageState({OnDemandRuleState? ruleState, this.ssids = const []})
    : ruleState = ruleState ?? OnDemandRuleState();

  OnDemandRulePageState _copy() {
    return OnDemandRulePageState(ruleState: ruleState, ssids: List.from(ssids));
  }
}
...
class OnDemandRuleController extends Cubit<OnDemandRulePageState> {
  ...
  void _initParams() {
    ...
    final controllers = ssids.map((e) => TextEditingController(text: e));
