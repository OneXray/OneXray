...
class XrayProfileListPage extends StatelessWidget {
      ...
      child: BlocBuilder<XrayProfileListController, XrayProfileListPageState>(
        builder: (context, state) {
          ...
          return Scaffold(
            appBar: AppBar(
              title: Text(
                AppLocalizations.of(context)!.xrayProfileListPageTitle,
              ),
              actions: [
                IconButton(
    ...
    return DefaultTextStyle.merge(
      ...
      child: Column(
        children: [
          ...
          Expanded(child: _xrayProfileList(context, controller, state)),
  ...
  Widget _search(BuildContext context, XrayProfileListController controller) {
    ...
    if (simpleConfigs.isNotEmpty) {
      rows.add(
        DataListSectionHeader(
          title: _sectionTitle(
            AppLocalizations.of(context)!.xrayProfileListPageSimple,
  ...
  Widget _simpleConfigRow(
    ...
    return ConfigRowView(
      data: data,
      status: data.id == state.xrayProfileId
          ? ConfigRowStatus.selected
          : ConfigRowStatus.unselected,
      moreMenus: [IconMenuId.edit],
      tapCallback: () => controller.updateXrayProfileId(context, data.id),
    );
  }
  ...
  Widget _configRow(
    ...
    final data = item.config;
    final selected = data.id == state.xrayProfileId;
    return ConfigRowView(
      data: data,
