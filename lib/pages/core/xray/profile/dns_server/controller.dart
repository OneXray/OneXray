...
class DnsServerPageState {
  ...
  DnsServerPageState({required this.serverState, this.version = 0});

  factory DnsServerPageState.initial() =>
      DnsServerPageState(serverState: DnsServerState());

  DnsServerPageState bumped() =>
...
class DnsServerController extends Cubit<DnsServerPageState> {
  ...
  void _initParams() {
    ...
    _initInput(initS);
    _initInputs(initS);
    emit(DnsServerPageState(serverState: initS, version: 1));
  }
  ...
  Future<void> importDomain(BuildContext context) async {
    ...
    final codes = await context.pushScoped<Set<String>>(
