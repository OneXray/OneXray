...
class OutboundsPageState {
  final OutboundsState outboundsState;
  final int version;

  const OutboundsPageState({required this.outboundsState, this.version = 0});

  factory OutboundsPageState.initial() =>
      OutboundsPageState(outboundsState: OutboundsState());

  OutboundsPageState bumped() =>
      OutboundsPageState(outboundsState: outboundsState, version: version + 1);
}

class OutboundsController extends Cubit<OutboundsPageState> {
  final OutboundsParams params;
  OutboundsController(this.params) : super(OutboundsPageState.initial()) {
    _initParams();
  }

  void _initParams() {
    emit(OutboundsPageState(outboundsState: params.state, version: 1));
  }
