...
class FakeDnsPageState {
  final FakeDnsPoolsState fakeDnsState;
  final int version;

  const FakeDnsPageState({required this.fakeDnsState, this.version = 0});

  factory FakeDnsPageState.initial() =>
      FakeDnsPageState(fakeDnsState: FakeDnsPoolsState());

  FakeDnsPageState bumped() =>
      FakeDnsPageState(fakeDnsState: fakeDnsState, version: version + 1);
}

class FakeDnsController extends Cubit<FakeDnsPageState> {
  final FakeDnsParams params;
  FakeDnsController(this.params) : super(FakeDnsPageState.initial()) {
    _initParams();
  }
  ...
  void _initParams() {
    ...
    _initInput(initS);
    emit(FakeDnsPageState(fakeDnsState: initS, version: 1));
  }
